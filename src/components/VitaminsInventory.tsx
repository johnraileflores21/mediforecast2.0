...
import ModalAdd from "./ModalAddVitamin";
import ModalEditVitamin from "./ModalEditVitamins";
import ModalViewVitamin from "./ModalViewVitamins";
...
const VitaminsInventory = () => {
    ...
    const fetchData = async () => {
        try {
            const querySnapshot = await getDocs(collection(db, "Vitamins"));
    ...
    const confirmDelete = async () => {
        if (deleteId) {
            try {
                await deleteDoc(doc(db, "Vitamins", deleteId));
    ...
    const handleSort = () => {
        const sortedData = [...userData].sort((a, b) => {
            if (a.vitaminName < b.vitaminName)
    ...
    return (
        <>
            <h1 className="text-xl text-center font-bold ">Vitamins</h1>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-300">
                                <tr>
                                    ...
                                    <th className="px-6 py-6 text-center border text-xs font-bold text-black uppercase tracking-wider">
                                        Stock
                                    </th>
                                    <th className="px-6 py-6 text-center border text-xs font-bold text-black uppercase tracking-wider">
                                        Description
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {currentItems.map((vitamin) => (
                                    <tr
                                        key={vitamin.id}
                                        className="relative"
                                        onMouseEnter={() =>
                                            setExpandedRow(vitamin.id)
                                        }
                                        onMouseLeave={() =>
                                            setExpandedRow(null)
                                        }
...
export default VitaminsInventory;
